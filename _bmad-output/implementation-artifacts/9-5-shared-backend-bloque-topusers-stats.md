...
- Task 1 (AC1): `StatsTopUserSchema` + required `topUsers` key with a `count DESC, authorId ASC` ordering `superRefine` added to `stats.ts`. Tests-first: 9 new cases in `stats.test.ts` confirmed red (schema/key did not exist), then green after implementation. Full file: 50/50 passing.
...
- Task 3 (AC3): `statsService.getStats` wires `getTopUsers` into the existing `Promise.all`, gated on `emptyScope` (channel-scoped, unlike `countUserAgentQueries`). Tests-first: 3 new cases in `statsService.test.ts` (empty-scope skip + `topUsers: []`, pass-through, misordered-fake throws via the outgoing `.parse()`) confirmed red then green. Full file: 10/10 passing.
